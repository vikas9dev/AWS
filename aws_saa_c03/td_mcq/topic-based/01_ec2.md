...
* ❌ **All instances in one AZ** → Not fault-tolerant.
* ❌ **Min = 2 across 2 AZs (1 per AZ)** → AZ failure leaves only **1 instance**.
* ❌ **Max = 4** → Does not meet peak requirement of 6 instances.
...
* ❌ **20 instances per region** – This is outdated; limits are vCPU-based, not instance-count based.
* ❌ **20 instances per Availability Zone** – No such default AZ-level instance limit exists.
* ❌ **EC2 API issue** – Re-sending requests will fail again until the quota issue is resolved.
